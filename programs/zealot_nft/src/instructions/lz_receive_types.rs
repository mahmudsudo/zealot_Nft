...
pub struct LzReceiveTypes<'info> {
    ...
    #[account(address = ONft_config.token_mint)]
...
impl LzReceiveTypes<'_> {
    pub fn apply(
        ...
        let ONft = &ctx.accounts.ONft_config;

        let (peer, _) = Pubkey::find_program_address(
            &[PEER_SEED, &ONft.key().to_bytes(), &params.src_eid.to_be_bytes()],
            ctx.program_id,
        );
        ...
        // account 2..3
        let (ONft_config, _) = Pubkey::find_program_address(
            &[ONft_SEED, &get_ONft_config_seed(&ONft).to_bytes()],
            ctx.program_id,
        );
        let token_escrow = if let ONftConfigExt::Adapter(token_escrow) = ONft.ext {
        ...
        accounts.extend_from_slice(&[
            LzAccount { pubkey: ONft_config, is_signer: false, is_writable: false }, // 2
            token_escrow,                                                           // 3
        ]);
        ...
        let token_dest = get_associated_token_address_with_program_id(
            &to_address,
            &ctx.accounts.ONft_config.token_mint,
        ...
        accounts.extend_from_slice(&[
            ...
            LzAccount { pubkey: ONft.token_mint, is_signer: false, is_writable: true }, // 6
            LzAccount { pubkey: ONft.token_program, is_signer: false, is_writable: false }, // 7
        ...
        let endpoint_program = ctx.accounts.ONft_config.endpoint_program;
